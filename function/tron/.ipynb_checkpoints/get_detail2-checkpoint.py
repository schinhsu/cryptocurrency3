...
def get_tx_by_hash(tronObj,txid):
    ...
    if 'transfersAllList' in res.keys():
        ...
        txlist[['BlockNo','TxID','Date','TxFee']] = [[res['block'],res['hash'],
                                                             pandas.to_datetime(res['timestamp'],unit='ms'),
        ...
        blockNo,txid,date,fee = res['block'],res['hash'],pandas.to_datetime(res['timestamp'],unit='ms'),res['cost']['energy_fee']/(10**6)
